...
if __name__ == "__main__":
    ...
    cmdline_parser.add_argument('-e', '--exon_file',
                                ...
                                required = True,
                                type=str)
    cmdline_parser.add_argument('-b', '--baseline_file',
                                default="",
                                help='output random baseline file',
                                required = True,
                                type=str)
    cmdline_parser.add_argument('-a', '--peak_sequences',
                                default="",
                                help='output peak sequencs',
                                required = True,
                                type=str)
    ...
    oops_seq_folder = glob.glob(args.oops_seq_folder + "*")
    oops_seq_folder_ids = [o.split("|")[0].split("/")[-1] for o in oops_seq_folder]
    ...
    for enum, line_ in enumerate(dms_file):
    
        if line_[0] == ">":
        
            
            oops_search = line_[1:-1]

            if oops_search not in oops_seq_folder_ids: continue
            
            oops_data = oops_seq_folder[oops_seq_folder_ids.index(oops_search)]
            ...
            if selected_region == []: continue

            
            seq = dms_file[enum+1][:-1]
            ...
            signal_list = []     
            sequence_list = []     
            for enum, signal in enumerate(signal_fade_out):
                if signal == 0:
                    signal_list.append(seq[enum])
                elif signal == 1:
                    if signal_list != []:
                        sequence_list.append("".join(signal_list))
                        signal_list = []
                        
            extracted_sequences.extend(sequence_list)



    random_baseline = []
    for subseq in extracted_sequences:
        string_copy = list(subseq)
        random.shuffle(string_copy)
        random_baseline.append("".join(string_copy))
            
    file_ = open(args.peak_sequences, "w")
    for enum, sequence in enumerate(extracted_sequences):
        file_.write(">" + str(enum) + "\n")
        file_.write(sequence)
        file_.write("\n")
        
    file_.close()
    file_ = open(args.baseline_file, "w")
    for enum, sequence in enumerate(random_baseline):
        file_.write(">" + str(enum) + "\n")
        file_.write(sequence)
        file_.write("\n")
    file_.close()
