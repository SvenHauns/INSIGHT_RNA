                ...
                if junction_list_right == []:
                    ...
                    for enum, half in enumerate(junction_list_right):
                         ...
                         if junction_list_middle != []:
                              middle_window = [window for window in junction_list_middle if window[-2] == half[-1] and window[-3] == half[-2]+1]
                         
                         #print(middle_window)
                         if middle_window == []: left_window = [window for window in junction_list_left if window[-1] == half[-1] and window[-2] == half[-2]+1]
                         else: left_window = [window for window in junction_list_left if window[-1] == middle_window[0][-1] and window[-2] == middle_window[0][-3]+1]
                         ...
                         if left_window == []: 
                             ...
                             if set_columns: columns[tid].append(half[1])
                             hisat1_counts[tid].append(half[0])
            ...
            junction_list_middle = []
             ...
             if (last_tid == tid or last_tid == None):
                ...
                if junc.split("_")[1] == "left":
                    ...
                    junction_list_left.append([current_length, int(line.split("\t")[1]), float(line.split("\t")[6][:-1]), exon_id, junc_id])
                elif junc.split("_")[1] == "right":
                    junc_id =  int(junc.split("_")[-1])
                    exon_id =  int(junc.split("_")[2])
                    junction_list_right.append([current_length, int(line.split("\t")[1]), float(line.split("\t")[6][:-1]), exon_id,  junc_id])
                elif junc.split("_")[1] == "middle":
                    junc_id_right =  int(junc.split("_")[-1])
                    junc_id_left =  int(junc.split("_")[-2])
                    exon_id =  int(junc.split("_")[2])
                    
                    junction_list_middle.append([current_length, int(line.split("\t")[1]), float(line.split("\t")[6][:-1]), exon_id,  junc_id_left, junc_id_right])
